...
ค่า environment เริ่มจาก `.env.example` แล้วสร้าง `.env.local` ในเครื่องพัฒนา (ต้องใส่ค่า Supabase จริง ดู `docs/13-setup-free-tier.md`)

## การเข้าสู่ระบบ

ระบบใช้ **username/password** (ไม่ใช่อีเมล) — โดย Supabase Auth ยังเก็บอีเมลเบื้องหลัง แต่ผู้ใช้ล็อกอินด้วย username สั้น ๆ ระบบจะแปลง username เป็นอีเมลฝั่งเซิร์ฟเวอร์ตอนล็อกอิน (อีเมลไม่ถูกส่งไปฝั่ง browser)
