...
import { fetchMonitorEvents, fetchMonitorTeam } from "@/lib/monitor/client";
...
export function ClassicMonitor() {
  ...
  const [team, setTeam] = useState<MonitorPerson[]>([]);
  const eventsRef = useRef<MonitorEvent[]>([]);
  eventsRef.current = monitorEvents;

  // Live monitor feed: load on mount + refresh every 60s (monitor-safe RPC).
  useEffect(() => {
    let active = true;
    const load = async () => {
      const [ev, tm] = await Promise.all([fetchMonitorEvents(), fetchMonitorTeam()]);
      if (!active) return;
      setMonitorEvents(ev);
      setTeam(tm);
      setRefreshIn(60);
  ...
  const tickerItems = useMemo(
    () => monitorEvents.map((e) => `📢 ${e.startTime} • ${e.title} • ${e.locationName}`),
    [monitorEvents],
  );
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      if (sec % 6 === 0 && count > 1) setActiveIdx((i) => (i + 1) % count);
  ...
  useEffect(() => {
    ...
    function boot(THREE: any) {
      ...
      eventsRef.current.forEach((ev, i) => {
        const col = ev.hasChanges ? 0xff9a5c : ev.allAck ? 0x46d38a : 0x7af0e0;
    ...
    // Rebuild the scene when the number of live events changes.
  }, [reduced, monitorEvents.length]);
  ...
  function renderChannel() {
    switch (ch.id) {
      case "agenda": return <ChannelAgenda events={monitorEvents} activeIdx={activeIdx} tip={PR_TIPS[tipIdx]} />;
      ...
      case "team": return <ChannelTeam team={team} />;
      default: return null;
    }
  }
